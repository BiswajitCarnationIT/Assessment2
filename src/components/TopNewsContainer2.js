...
const TopNewContainer = () => {
    ...
    console.log("hi from TopContainer", newsData);
    const topNienty = []
    let topNientyBody = []
    ...
    if(newsData.news[1]){
        ...
        .then(response => {
            ...
            // topNientyBody[i] = 1
            //console.log('hi',topNientyBody[i])
        })
    }
    ...
    setTimeout(console.log(topNientyBody[1]),5000)
    console.log(topNientyBody[1])
    topNientyBody.map(user => console.log("user"))
    console.log('user.by')
    ...
    return(
        <div>
            ...
            {/* <h1>{newsData.news.map(user => <div><h3>{user}</h3></div>)}</h1>  */}
